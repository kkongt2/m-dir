...
class ExplorerPane(QWidget):
    ...
    def _on_context_menu(self, pos):
        ...
        # A selection can remain highlighted while the user right-clicks the blank
        # area below the rows.  In that case this must be a folder-background menu,
        # not the context menu for the stale selected item.  Item shell extensions
        # can otherwise contribute a second command targeting that item's parent.
        clicked_index = self.view.indexAt(pos)
        paths = self._selected_paths() if clicked_index.isValid() else []
